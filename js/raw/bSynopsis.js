...
(function($, window, document, undefined) {
	...
	Plugin.Layer.prototype._initLayerData = function(queries) {
		...
		$.each(this.options.nodeFilters, function(i, filter) {
			that.nodeFilterNames.push(i);
		});
		$.each(this.options.tileFilters, function(i, filter) {
			that.tileFilterNames.push(i);
		});
		this._addFilterSelection();
		// @endif
	...
	Plugin.Layer.prototype.switchFilterState = function(filterName) {
		...
		if (filterName === "on") {
			...
			$.each(that.options.tileFilters, function(i, filter) {
		...
		var nodeFilters = {};
		var tileFilters = {};
		$.each(this.nodeFilterNames, function(i, val) {
			if (that.options.nodeFilters[val]) {
				nodeFilters[val] = that.options.nodeFilters[val];
			}
		});
		$.each(this.tileFilterNames, function(i, val) {
			if (that.options.tileFilters[val]) {
				tileFilters[val] = that.options.tileFilters[val];
			}
		});

		this.view.clearView(function() {
			that.view.paint(that.getTilesToPaint(args.addedNodes, nodeFilters, tileFilters));
		});
	};
	// @endif
